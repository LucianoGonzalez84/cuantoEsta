...
function initBannerCarousel() {
    ...
    function goToSlide(index) {
        ...
        const offset = -bannerCurrentSlide * 100;
    ...
    dots.forEach((dot, index) => {
        dot.addEventListener('click', () => {
            ...
            bannerAutoplayInterval = setInterval(() => {
                const nextSlide = (bannerCurrentSlide + 1) % bannerTotalSlides;
                goToSlide(nextSlide);
            }, 3000);
        });
    });
    ...
    track.addEventListener('touchend', (e) => {
        ...
        if (Math.abs(diff) > 50) {
            if (diff > 0) {
                goToSlide(Math.min(bannerCurrentSlide + 1, bannerTotalSlides - 1));
            } else {
                goToSlide(Math.max(bannerCurrentSlide - 1, 0));
            }
        }
        bannerAutoplayInterval = setInterval(() => {
            const nextSlide = (bannerCurrentSlide + 1) % bannerTotalSlides;
...
window.addEventListener('resize', () => {
    ...
    bannerResizeTimeout = setTimeout(initBannerCarousel, 250);
});
